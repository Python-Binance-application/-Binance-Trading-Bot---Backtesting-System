...
def get_df_update(df_all):
        ...
        step=0.02, max_step=0.2).psar().loc[df_all.index].round(2)
        # Calcola le Bande di Bollinger (20 periodi, deviazione standard 2)
        bb = ta.volatility.BollingerBands(close=df_all['Close'], window=20, window_dev=2)

        # Aggiungi le bande al DataFrame
        df_all['BB_upper'] = bb.bollinger_hband().round(2)
        df_all['BB_middle'] = bb.bollinger_mavg().round(2)
        df_all['BB_lower'] = bb.bollinger_lband().round(2)
        return df_all
