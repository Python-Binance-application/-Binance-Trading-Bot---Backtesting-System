...
def get_df_update(df_all):
        ...
        df_all['Ema_fast'] = ta.trend.EMAIndicator(close=df_all['Close'], window=7).ema_indicator().loc[df_all.index].round(2)
        df_all['Ema_slow'] = ta.trend.EMAIndicator(close=df_all['Close'], window=21).ema_indicator().loc[df_all.index].round(2)
        df_all['Rsi'] = ta.momentum.RSIIndicator(close=df_all['Close'], window=14).rsi().loc[df_all.index].round(2)
